...
uint8_t CommonBuff[128];

uint8_t bCSCMtx_RGB2YUV_ITU601_16_235[] = {0x00, 0x80, 0x00, 0xB2, 0x04, 0x65, 0x02, 0xE9, 0x00, 0x93, 0x3C,
                                           0x18, 0x04, 0x55, 0x3F, 0x49, 0x3D, 0x9F, 0x3E, 0x18, 0x04};

uint8_t bCSCMtx_RGB2YUV_ITU601_0_255[] = {0x10, 0x80, 0x10, 0x09, 0x04, 0x0E, 0x02, 0xC9, 0x00, 0x0F, 0x3D,
                                          0x84, 0x03, 0x6D, 0x3F, 0xAB, 0x3D, 0xD1, 0x3E, 0x84, 0x03};

uint8_t bCSCMtx_RGB2YUV_ITU709_16_235[] = {0x00, 0x80, 0x00, 0xB8, 0x05, 0xB4, 0x01, 0x94, 0x00, 0x4a, 0x3C,
                                           0x17, 0x04, 0x9F, 0x3F, 0xD9, 0x3C, 0x10, 0x3F, 0x17, 0x04};

uint8_t bCSCMtx_RGB2YUV_ITU709_0_255[] = {0x10, 0x80, 0x10, 0xEa, 0x04, 0x77, 0x01, 0x7F, 0x00, 0xD0, 0x3C,
                                          0x83, 0x03, 0xAD, 0x3F, 0x4B, 0x3D, 0x32, 0x3F, 0x83, 0x03};

uint8_t bCSCMtx_YUV2RGB_ITU601_16_235[] = {0x00, 0x00, 0x00, 0x00, 0x08, 0x6B, 0x3A, 0x50, 0x3D, 0x00, 0x08,
                                           0xF5, 0x0A, 0x02, 0x00, 0x00, 0x08, 0xFD, 0x3F, 0xDA, 0x0D};

uint8_t bCSCMtx_YUV2RGB_ITU601_0_255[] = {0x04, 0x00, 0xA7, 0x4F, 0x09, 0x81, 0x39, 0xDD, 0x3C, 0x4F, 0x09,
                                          0xC4, 0x0C, 0x01, 0x00, 0x4F, 0x09, 0xFD, 0x3F, 0x1F, 0x10};

uint8_t bCSCMtx_YUV2RGB_ITU709_16_235[] = {0x00, 0x00, 0x00, 0x00, 0x08, 0x55, 0x3C, 0x88, 0x3E, 0x00, 0x08,
                                           0x51, 0x0C, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x84, 0x0E};

uint8_t bCSCMtx_YUV2RGB_ITU709_0_255[] = {0x04, 0x00, 0xA7, 0x4F, 0x09, 0xBA, 0x3B, 0x4B, 0x3E, 0x4F, 0x09,
...
void HDMITX_LoadRegSetting(display_handle_t *handle, RegSetTable_t *table, int table_sz)
{
    ...
    for (i = 0; i < table_sz / sizeof(RegSetTable_t); i++)
    {
        if (table[i].invAndMask == 0xFF)
        {
...
{
    HDMITX_SetI2C_Byte(handle, 0x0F, 0x03, value & 0x03);
}

void DumpHDMITXReg(display_handle_t *handle)
{
...
void HDMITX_ClearDDCFIFO(display_handle_t *handle)
{
...
void HDMITX_AbortDDC(display_handle_t *handle)
{
    uint8_t SWReset;
    uint8_t uc, timeout, i;
...
bool getHDMITX_EDIDBytes(display_handle_t *handle, uint8_t *pData, uint8_t bSegment, uint8_t offset, uint16_t Count)
{
...
bool getHDMITX_EDIDBlock(display_handle_t *handle, int32_t EDIDBlockID, uint8_t *pEDIDData)
{
    if (!pEDIDData)
    {
        return false;
    ...
    for (offset = 4; offset < End;)
    {
        ...
        switch (tag)
        {
            ...
            case 0x03: // Vendor Specific Data Block ;
                offset++;
                RxCapability.IEEEOUI = (uint32_t)pCEAEDID[offset + 2];
                RxCapability.IEEEOUI <<= 8;
                RxCapability.IEEEOUI += (uint32_t)pCEAEDID[offset + 1];
                RxCapability.IEEEOUI <<= 8;
...
uint8_t ParseEDID(display_handle_t *handle)
{
    ...
    if (EDID_Buf[0] != 0x00 || EDID_Buf[1] != 0xFF || EDID_Buf[2] != 0xFF || EDID_Buf[3] != 0xFF ||
        EDID_Buf[4] != 0xFF || EDID_Buf[5] != 0xFF || EDID_Buf[6] != 0xFF || EDID_Buf[7] != 0x00)
    {
        return false;
    }
...
{
    int i;
    int sum = 0, RCLKCNT, TimeLoMax, retry = 5;
    uint8_t value;
...
{
    uint8_t AudioEnable = 0, AudioFormat = 0, bTDMSetting;
...
{
    static uint8_t ucIEC60958ChStat[5];
    uint8_t Fs;
...
{
    uint8_t Reg06, Reg07, Reg08, RegEE;
