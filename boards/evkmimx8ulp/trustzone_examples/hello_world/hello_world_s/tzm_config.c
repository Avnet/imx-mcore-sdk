    ...
    for (i = 0; i < hwConfig.mrcNumber; i++)
    {
        ...
        for (j = 0; j < hwConfig.domainNumber; j++)
        {
            ...
            for (m = 0U; m < n; m++)
            {
                ...
                mrcRegionConfig.endAddr   = (mrc_start_addr[i] + (mrc_end_addr[i] - mrc_start_addr[i]) / n * (m + 1U) - 4 * 1024) &
                    (~(4*1024 - 1));

                if(mrcRegionConfig.startAddr < SAU_REGION_0_BASE ||
                        mrcRegionConfig.endAddr > SAU_REGION_0_END ){
                    mrcRegionConfig.nseEnable = false;
        ...
        for (j = 0U; j < hwConfig.domainNumber; j++)
        {
            ...
            for (m = 0U; m < 4; m++)
            {
                ...
                for (n = 0U; n < mbcHwConfig.blockNum; n++)
                {
                    ...
                    if(i == 0 && j == 0 && m == 1){ /*mbc0 tcmu blocknum[4-5] for non-secure data2*/
                        if(n > 3 && n < 8){
                            mbcBlockConfig.nseEnable = true;
                        }else{
                            mbcBlockConfig.nseEnable = false;
                        }
                    }else if(i == 0 && j == 0 && m == 0){ /*mbc0 tcml blocknum[0-3] for non-secure data*/
                        if( n >=4 && n < 8 ){
                            mbcBlockConfig.nseEnable = true;
                        }else{
                            mbcBlockConfig.nseEnable = false;
...
void BOARD_InitTrustZone()
{
    ...
    /* Region end address */
    SAU->RLAR = (SAU_REGION_5_END & SAU_RLAR_LADDR_Msk)
        | ((0U << SAU_RLAR_NSC_Pos) & SAU_RLAR_NSC_Msk)
        | ((1U << SAU_RLAR_ENABLE_Pos) & SAU_RLAR_ENABLE_Msk);
    ...
    /* Region end address */
    SAU->RLAR = (SAU_REGION_6_END & SAU_RLAR_LADDR_Msk)
        | ((0U << SAU_RLAR_NSC_Pos) & SAU_RLAR_NSC_Msk)
        | ((1U << SAU_RLAR_ENABLE_Pos) & SAU_RLAR_ENABLE_Msk);
