 ...
 * Definitions
 *****************************************************************************/
#define FSL_TRDC_DRIVER_VERSION (MAKE_VERSION(2, 0, 1))
...
{
    uint32_t domainId : 3U;       /*!< Domain ID. */
    uint32_t : 1U;                /*!< Reserved. */
    uint32_t domainIdSelect : 2U; /*!< Domain ID select method, see @ref trdc_did_sel_t. */
    uint32_t : 8U;                /*!< Reserved. */
    uint32_t secureAttr : 2U; /*!< Secure attribute, see @ref trdc_secure_attr_t. */ // TODO cannot be modified
...
{
    uint32_t blockCount : 15U;    /*!< Block count of the Flash Logic Window in 32KByte blocks. */
    uint32_t arrayBaseAddr : 17U; /*!< Flash array base address of the Flash Logical Window. */
    uint32_t : 30U;               /*!< Reserved. */
    uint32_t lock : 1U;           /*!< Disable writes to FLW registers. */
    uint32_t enable : 1U;         /*!< Enable FLW function. */
} trdc_flw_config_t;

/* Domain error check and clear definitions */
/*!
 * @brief TRDC controller definition for domain error check.
 */
...
typedef struct _trdc_mbc_memory_block_config
{
    ...
    uint8_t mbcIdx : 4U;                     /*!< The index of the MBC for this configuration to take effect. */
    uint8_t domainIdx : 8U;                  /*!< The index of the domain for this configuration to take effect. */
    uint8_t slaveMemoryIdx : 8U; /*!< The index of the slave memory for this configuration to take effect. */
    uint8_t memoryBlockIdx : 8U; /*!< The index of the memory block for this configuration to take effect. */
 ...
 */
static inline void TRDC_LockMasterDomainAssignment(TRDC_Type *base,
                                                   uint8_t master) // TODO wait for header file trdc_master_t definition
{
 ...
 */
static inline void TRDC_SetMasterDomainAssignmentValid(TRDC_Type *base,
                                                       uint8_t master,
                                                       bool valid) // TODO wait for header file trdc_master_t definition
{
    /* Make sure in the master range. */
    assert((uint32_t)master < ((base->TRDC_HWCFG0 & TRDC_TRDC_HWCFG0_NMSTR_MASK) >> TRDC_TRDC_HWCFG0_NMSTR_SHIFT));
...
{
    return (base->TRDC_FLW_PBASE & TRDC_TRDC_FLW_PBASE_PBASE_MASK) >> TRDC_TRDC_FLW_PBASE_PBASE_SHIFT;
}
...
{
    return ((base->MRCFG[mrcIdx] & TRDC_MRCFG_NMRGD_MASK) >> TRDC_MRCFG_NMRGD_SHIFT);
}
