...
/* Clock name of TRDC. */
// static const clock_ip_name_t s_trdcMbcClock[] = TRDC_MBC_CLOCKS; //TODO MBC instance clock incorrect in RM and header
                               ...
                               uint8_t slvIdx)
{
    ...
    volatile const uint32_t *reg = &(base->CFG[mbcIdx][slvIdx / 2U]);

    config->blockNum = ((*reg >> ((slvIdx % 2U) * 16U)) & TRDC_CFG_SLV0_NMBLK_MASK) >> TRDC_CFG_SLV0_NMBLK_SHIFT;
    config->blockSize =
        1U << (((*reg >> ((slvIdx % 2U) * 16U)) & TRDC_CFG_SLV0_BLKSZL2_MASK) >> TRDC_CFG_SLV1_BLKSZL2_SHIFT);
}
...
void TRDC_GetDefaultFlashLogicalWindowConfig(trdc_flw_config_t *flwConfiguration)
{
    ...
    flwConfiguration->lock   = (uint32_t) true;
    flwConfiguration->enable = (uint32_t) true;
}
...
void TRDC_SetFlashLogicalWindow(TRDC_Type *base, const trdc_flw_config_t *flwConfiguration)
{
    ...
    base->TRDC_FLW_ABASE = flwConfiguration->arrayBaseAddr << 15U;
...
void TRDC_MbcNseClearAll(TRDC_Type *base, uint8_t mbcIdx, uint8_t domainMask, uint8_t memoryMask)
{
    ...
    uint32_t mask = TRDC_MBC_NSE_BLK_CLR_ALL_DID_SEL(domainMask) | TRDC_MBC_NSE_BLK_CLR_ALL_MEMSEL(memoryMask);

    base->MBC_INDEX[mbcIdx].MBC_NSE_BLK_CLR_ALL = mask;
}
...
void TRDC_MbcSetMemoryBlockConfig(TRDC_Type *base, const trdc_mbc_memory_block_config_t *config)
{
    ...
    uint8_t shift           = 4U * (config->memoryBlockIdx % 8U);
    ...
    regAddr += (TRDC_MBC_DOMAIN_INCREMENT * config->domainIdx + slaveIncrement) +
               (config->memoryBlockIdx / 8U) * sizeof(uint32_t);
